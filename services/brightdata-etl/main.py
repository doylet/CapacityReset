"""
BrightData ETL Service - Transform raw LinkedIn job postings into structured BigQuery table

HTTP endpoint that executes the transformation MERGE query.
"""
...
def main(request):
    """
    HTTP Cloud Function to transform LinkedIn job postings.
    
    Executes the MERGE query that transforms raw JSON from scrape_requests
    into the structured job_postings table.
    ...
    try:
        ...
        merge_query = f"""
        MERGE `{project_id}.brightdata_jobs.job_postings` AS target
        USING (
          ...
          WHERE status = '200'
            AND JSON_VALUE(job, '$.job_posting_id') IS NOT NULL
        ) AS source
        ...
        print(f"ETL transformation completed. Rows affected: {num_dml_affected_rows}")
        
        return {
            "status": "success",
            "rows_affected": num_dml_affected_rows,
            "message": f"Successfully transformed {num_dml_affected_rows} job postings"
        }, 200
